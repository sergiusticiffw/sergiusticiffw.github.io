...
import { extractHashtags, addTagToText, removeTagFromText, hasTagInText } from '@utils/utils';
...
export const useTags = ({
  ...
  const normalize = useMemo(() => customNormalizeTag || normalizeTag, [customNormalizeTag]);
  ...
  useEffect(() => {
    ...
    if (!descriptionChanged && formType === 'edit' && prevDescriptionRef.current) {
      return;
    }
    ...
    if (formType === 'edit' && description && suggestionsRef.current.length > 0) {
   ...
   */
  const handleTagClick = useCallback((tag: string): string => {
  ...
  const isTagSelected = (tag: string): boolean => {
    const normalizedTag = normalize(tag);
    return selectedTags.includes(normalizedTag) || hasTagInText(description, normalizedTag);
  };
