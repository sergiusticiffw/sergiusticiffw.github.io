  ...
  const formattedIncomeData = useMemo(
    () =>
      formatDataForChart(
        ...
        view?.totalsPerYearAndMonth ?? {},
        monthNames
      ),
    [totalIncomePerYearAndMonth, view?.totalsPerYearAndMonth, monthNames]
  );
  const options: Highcharts.Options = useMemo(
    () => ({
      chart: { type: 'line', zooming: { type: 'x' } },
      title: { text: t('charts.monthlySavingsAcrossYears') },
...
export default React.memo(MonthlySavingsTrend);
