...
const AmortizationTable: React.FC<AmortizationTableProps> = ({
  ...
  return (
    <div className={AMORT_ROOT}>
      ...
      <div className={`amort-body ${AMORT_BODY}`}>
        <div className={AMORT_BODY_SPLIT}>
          ...
          >
            <table
              className={`${AMORT_ROW_TABLE} min-w-0 [&_td]:hover:!bg-[color-mix(in_srgb,var(--color-app-accent)_8%,transparent)]`}
              style={{ width: firstColWidth }}
            >
              <tbody>
                {rows.map((row) => {
                  ...
                  return (
                    <tr
                      key={`${row.id}-first`}
                      className={getFirstColRowDivClasses(row)}
                    >
                      <td
          ...
          <div ref={bodyHorizontalRef} className={AMORT_BODY_HORIZONTAL}>
            <table className={AMORT_ROW_TABLE}>
              <tbody>
                {rows.map((row) => (
                  <tr key={row.id} className={getRowDivClasses(row)}>
                    {row
                      .getVisibleCells()
                      .slice(1)
                      .map((cell) => (
