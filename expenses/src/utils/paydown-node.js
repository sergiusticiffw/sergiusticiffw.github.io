  ...
  this.calculate = function (
    ...
    if (init_obj.hasOwnProperty('round_values')) {
      if (init_obj.round_values) {
        sum_of_installments = func_round(
          interests + reductions - final_interest
        );
      ...
      // round_values default value: true
...
*/
    ...
    if (!principal) {
      throw new Error('this.get_period_interests invalid parameter: principal');
    }
    // rate is allowed to be zero
    if (!start_date) {
      throw new Error(
        'this.get_period_interests invalid parameter: start_date'
      );
    }
    if (!end_date) {
      throw new Error('this.get_period_interests invalid parameter: end_date');
    }

    if (date_to_integer(start_date) > date_to_integer(end_date)) {
      throw new Error(
        'this.get_period_interests invalid date: start_date ' +
          start_date +
          ' is after end_date ' +
          end_date
      );
    ...
    if (this.latest_period_end_date) {
      var date_difference = calculate_day_count(
        this.latest_period_end_date,
        start_date,
        true
      );
      if (date_difference !== 1) {
        throw_unexpected_exception(
          'Date difference is not one day as expected, latest_period_end_date: ' +
            this.latest_period_end_date +
            ' start_date: ' +
            start_date
        );
    ...
    if (rate_event) {
      ...
      while (rate_event) {
        ...
        number_of_days = calculate_day_count(
          subperiod_start_date,
          rate_event_date,
          true
        );
        ...
        rate_event = this.get_first_event_after_date(
          'rate',
          rate_event_date,
          end_date
        );
      ...
      if (this.debug_logging_enabled) {
        this.debug_log(
          ...
          subperiod_interest / number_of_days
        );
      }
      sum_of_interests += subperiod_interest;
  ...
  this.get_first_event_after_date = function (property, date, boundary_date) {
    for (var index = 0; index < this.event_array.length; index++) {
      if (
        ...
        this.event_array[index].hasOwnProperty(property) &&
        date_to_integer(this.event_array[index].date) <=
          date_to_integer(boundary_date)
      ) {
  ...
  this.check_date = function (date, context) {
    ...
    if (typeof date !== 'string') {
      throw new Error(
        'this.check_date: ' + context + ' date must be of type string: ' + date
      );
    }

    if (!check_date_validity(date)) {
      throw new Error(
        'this.check_date: ' + context + ' date is invalid: ' + date
      );
    }
  };

  this.handle_last_payment = function (
    reduction,
    date,
    period_interest,
    fee = 0
  ...
  this.func_pay_installment = function (index, date_obj, installment, fee = 0) {
    ...
    if (this.latest_calculated_interest_date === this.event_array[index].date) {
      ...
      period_interest = this.get_period_interests(
        this.current_principal,
        this.current_rate,
        start_date,
        end_date
    ...
    if (reduction < 0) {
      ...
      throw new Error(
        'Exception: installment ' +
          this.round(installment) +
          ' is too small to cover the interest ' +
          this.round(period_interest) +
          ': ' +
          start_date +
          ' - ' +
  ...
  this.calculate_to_date = function (array_of_events, array_of_debug_prints) {
    ...
    if (typeof this.init.principal !== 'number' || isNaN(this.init.principal)) {
      throw new Error('this.calculate_to_date: principal must be number');
    }
    if (this.init.principal === 0) {
      throw new Error('this.calculate_to_date: principal is missing');
    }
    if (typeof this.init.rate !== 'number' || isNaN(this.init.rate)) {
      throw new Error('this.calculate_to_date: rate must be number');
    }
    ...
    this.latest_calculated_interest_date = date_obj
      .set_current(this.init.start_date)
      .get_prev();
    ...
    for (index = 0; index < this.event_array.length; index++) {
        ...
        if (this.event_array[index].payment_method === 'equal_installment') {
          ...
          throw new Error(
            'invalid payment method in event: ' +
              this.event_array[index].payment_method
          );
        }
      }
      ...
      if (this.event_array[index].hasOwnProperty('ending')) {
        if (
          ...
          final_interest = this.get_period_interests(
            this.current_principal,
            this.current_rate,
            date_obj
              .set_current(this.latest_calculated_interest_date)
    ...
    if (
      this.round(this.g_p_i_sum_of_interests) !==
      this.round(this.sum_of_interests)
    ) {
      throw_unexpected_exception(
        'Sum of interests mismatch: ' +
          this.round(this.g_p_i_sum_of_interests) +
          ' vs. ' +
          this.round(this.sum_of_interests)
      );
  ...
  this.set_init = function (data) {
    ...
    if (data.hasOwnProperty('recurring')) {
      this.current_recurring_payment = this.calculateRecurringAmount(data);
      ...
      if (
        !data.recurring.hasOwnProperty('payment_day') ||
        !number_is_valid(data.recurring.payment_day) ||
        data.recurring.payment_day < 1 ||
        data.recurring.payment_day > 31 ||
        !Number.isInteger(data.recurring.payment_day)
      ) {
        throw new Error(
          'this.set_init: invalid or missing first payment_day number'
        );
      }
  ...
  this.check_and_add_event = function (event) {
    ...
    if (event.hasOwnProperty('rate')) {
      if (isNaN(event.rate) || typeof event.rate !== 'number') {
        throw new Error(
          'this.check_and_add_event: invalid rate in event ' + event.date
        );
      }
    }
    ...
    if (event.hasOwnProperty('pay_single_fee')) {
      if (!number_is_valid(event.pay_single_fee)) {
        throw new Error(
          'this.check_and_add_event: invalid pay_single_fee in event ' +
            event.date
        );
      }
    }
  ...
  this.merge_events = function () {
    ...
    for (var index = 0; index < this.event_array.length - 1; index++) {
      if (this.event_array[index].hasOwnProperty('rate')) {
        this.rateHashMap[date_to_integer(this.event_array[index].date)] =
          this.event_array[index].rate;
      }

      if (
        date_to_integer(this.event_array[index].date) ===
        date_to_integer(this.event_array[index + 1].date)
      ) {
  ...
  this.check_first_payment_date = function () {
    ...
    if (
      date_to_integer(this.init.first_payment_date) <=
      date_to_integer(this.init.start_date)
    ) {
      throw new Error(
        'this.check_first_payment_date: first payment date must be after start date'
      );
    }
  };
  ...
  this.check_events = function () {
    for (var index = 0; index < this.event_array.length - 1; index++) {
      if (
        ...
        throw new Error(
          'this.check_events: event date (' +
            this.event_array[index].date +
            ') before start date not allowed'
        );
      }
    }
  };

  this.debug_log_period_start = function (
    start_date,
    totalNumberOfDays,
    principal,
    rate
  ...
  this.debug_log_subperiod = function (
    number_of_days,
    subperiod_interest,
    rate_event_date,
    new_rate
    ...
    this.debug_write(
      'Subperiod daily interest: ' +
        this.round(subperiod_interest / number_of_days)
    );
    this.debug_write(
      'Subperiod total interest: ' + this.round(subperiod_interest)
    );
    if (rate_event_date) {
      this.debug_write(
        'Rate changes ' + rate_event_date + ', new rate is ' + new_rate
      );
    }
  };
...
function _Days(init_date) {
  ...
  this.get_nth_month_nth_day = function (period_in_months, day_number) {
    if (
      period_in_months === 1 &&
      (day_number === 31 || (this.month === 1 && day_number > 28))
    ) {
...
function event_array_sorter(a, b) {
  if (date_to_integer(a.date) > date_to_integer(b.date)) {
    return 1;
  }
  if (date_to_integer(a.date) < date_to_integer(b.date)) {
    return -1;
...
function zero_fill_date(date) {
  if (typeof date !== 'string') {
    throw new Error('zero_fill_date illegal parameter type');
  }
