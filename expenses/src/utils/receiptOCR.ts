  ...
  try {
    ...
    await worker.setParameters({
      ...
      tessedit_char_whitelist:
        '0123456789.,€$RONLEIleiABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzĂăÂâÎîȘșȚț :/-', // Common receipt characters
    });
...
export function parseReceiptText(text: string): ExtractedReceiptData {
  ...
  let bestMatch: { amount: string; position: number; priority: number } | null =
    null;
    ...
    for (const match of matches) {
      ...
      if (!isNaN(numAmount) && numAmount >= 0.01 && numAmount <= 100000) {
        ...
        if (!bestMatch || priority < bestMatch.priority) {
          ...
          numAmount > parseFloat(bestMatch.amount)
        ) {
  ...
  if (lines.length > 2) {
    ...
    const isValidProductName = (text: string): boolean => {
      ...
      if (
        /card|visa|mastercard|terminal|rrn|autorizare|tranzactie|contactless|achitare/i.test(
          lowerText
        )
      ) {
        return false;
      ...
      const specialCharCount = (text.match(/[^a-zA-Z0-9\săâîșțĂÂÎȘȚ]/g) || [])
        .length;
    ...
    const extractProductName = (line: string): string | null => {
      ...
      cleaned = cleaned.replace(
        /^\d+\s*(?:buc\.?|buc)?\s*x\s*[\d,]+\s*\.?\s*\d*\s*(?:=\s*[\d,]+\s*\.?\s*\d*)?\s*/i,
        ''
      );
      ...
      cleaned = cleaned.replace(
        /\s+[\d,]+\s*\.?\s*\d*\s*(?:mdl|ron|lei|€|eur|b)\s*$/i,
        ''
      ); // Remove "32,00 RON" or "32,00 B"
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (productSectionStart === -1) {
        ...
        if (
          /^\d+\s*(?:x|buc\.?)\s*[\d,]/.test(trimmed) || // "4 x 8,00" or "1 buc. 156.00"
          /[a-zA-ZăâîșțĂÂÎȘȚ]{4,}\s+\d+\s*(?:x|buc\.?)/i.test(trimmed) || // "Product 1 buc"
          (/[a-zA-ZăâîșțĂÂÎȘȚ]{3,}/.test(trimmed) &&
            /\d+\.?\d*\s*[AB]?\s*$/.test(trimmed))
        ) {
          // "Product 32,00 B"
      ...
      if (productSectionStart !== -1 && productSectionEnd === -1) {
        if (
          /^total\s*:?|^tva\s*|livrare|card|visa|mastercard|terminal|rrn|autorizare|cu card|^total\s+[\d,]+/i.test(
            lowerTrimmed
          )
        ) {
          productSectionEnd = i;
    ...
    if (productSectionStart !== -1) {
      const endIndex =
        productSectionEnd !== -1 ? productSectionEnd : lines.length;
      ...
      for (let i = productSectionStart; i < endIndex; i++) {
        ...
        if (
          /^(?:total|tva|card|visa|nota|data|bon|fiscal|articole|preturi|romaniei|detalii|produs|cantitate)/i.test(
            trimmed
          )
        ) {
          continue;
    ...
    if (descriptionParts.length === 0) {
      for (let i = 0; i < lines.length; i++) {
        ...
        if (
          /^total\s*:?|^tva\s*|card|visa|mastercard|terminal|rrn|autorizare|cu card/i.test(
            lowerTrimmed
          )
        ) {
          break;
        ...
        if (
          trimmed.length < 4 ||
          /^(?:nota|data|total|suma|tva|platit|numerar|masa|ora|solution|solutie|am primit|comanda|pentru suma|farmacia|s\.r\.l\.|idno|inr|preturi|romaniei|bon fiscal|articole|detalii|produs|cantitate)/i.test(
            trimmed
          ) ||
