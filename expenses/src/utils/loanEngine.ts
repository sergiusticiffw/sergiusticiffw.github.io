...
export const recalculateAfterRateChange = (params: {
  ...
  if (currentPrincipal <= 0) {
    throw new Error('recalculateAfterRateChange: currentPrincipal must be positive');
  }
  if (currentPayment <= 0) {
    throw new Error('recalculateAfterRateChange: currentPayment must be positive');
  }
...
export const calculatePeriodInterest = (
  ...
  if (daysInPeriod < 0) {
    throw new Error('calculatePeriodInterest: daysInPeriod must be non-negative');
  }
