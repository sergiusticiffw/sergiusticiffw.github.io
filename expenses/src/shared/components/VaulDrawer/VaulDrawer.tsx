...
const VaulDrawer: React.FC<VaulDrawerProps> = ({
  ...
  /* Când tastatura se închide: curăță height/bottom setate de Vaul ca drawer-ul să revină la 85dvh */
  useEffect(() => {
  ...
  return (
    ...
    >
      <Drawer.Portal>
        ...
        <Drawer.Content
          className="fixed left-0 right-0 bottom-0 z-[10000] mx-auto flex h-[85dvh] max-h-[94dvh] w-full max-w-[640px] flex-col overflow-hidden rounded-t-2xl bg-[var(--color-app-bg)] outline-none transition-[height] duration-300 ease-in-out"
          style={
            keyboardVisible
          ...
          {footer && (
            <div
              className="modal-footer sticky bottom-0 shrink-0 border-t border-app-subtle bg-[var(--color-app-bg)] px-5 py-4 pb-[calc(1rem+env(safe-area-inset-bottom))] backdrop-blur-xl max-[480px]:px-4
                ...
                [&_.btn-submit]:bg-[linear-gradient(135deg,var(--color-app-accent),var(--color-app-accent-hover))] [&_.btn-submit]:shadow-[0_2px_8px_var(--color-app-accent-shadow),0_0_20px_var(--color-app-glow)]
