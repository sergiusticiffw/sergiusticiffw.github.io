import { categories, monthNames } from '@shared/utils/constants';
import {
  ...
  const newState = filtered.reduce(
    (accumulator: Accumulator, item: TransactionOrIncomeItem) => {
      ...
      if (cat !== undefined) {
        ...
        accumulator.categoryTotals[cat].name = categories[cat]?.label || '';
...
export const LoanReducer = (
  ...
  switch (action.type) {
    case 'SET_DATA':
      return {
        ...initialState,
        loans: action.loans ?? initialState.loans,
        payments: action.payments ?? initialState.payments,
